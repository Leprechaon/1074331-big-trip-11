...
const renderEvent = (eventListElement, event) => {
  ...
  render(eventDetailsComponent.getElement(), new EventOffersComponent(eventData).getElement(), RenderPosition.BEFOREEND);
  render(eventDetailsComponent.getElement(), new EventDestinationsComponent(event).getElement(), RenderPosition.BEFOREEND);
};
...
const renderHeader = () => {
  ...
  render(tripInfo.getElement(), new TripCostComponent().getElement(), RenderPosition.BEFOREEND);
...
const renderEventBoard = () => {
  ...
  render(tripEventsElement, new EventSortComponent().getElement(), RenderPosition.BEFOREEND);
  ...
  .map((eventDay, i) => {
    ...
    const tripEventsListElement = tripDayComponent.getElement().querySelector(`.trip-events__list`);
    eventDay.map((it) => renderEvent(tripEventsListElement, it)
    );
  });
};
